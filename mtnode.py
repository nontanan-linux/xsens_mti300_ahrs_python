...
class XSensDriver:
	def __init__(self):
		...
		# Enable config mode
		# try:
		self.mt = mtdevice.MTDevice(device, baudrate, timeout, False, False, False)
		...
		# print("Changing output configuration",self.mt.SetOutputConfiguration(output_config))
	...
	def spin_once(self):
		...
		def convert_quat(self, q, source, dest='ENU'):
		...
		# get data
		try:
			data = self.mt.read_measurement(None,None)
