...
class MTDevice(object):
	...
	def __init__(self, port="/dev/ttyUSB0", baudrate=115200, timeout=0.002, autoconf=False,
		...
		# state of the device
		print("Auto config : {}".format(autoconf))
		self.state = True
		if autoconf:
	...
	def write_msg(self, mid, data=b''):
		...
		# packet += struct.pack('!B', 0xFF & (-(sum(map(ord, packet)))))
		msg = packet
		start = time.time()
		...
		if self.verbose:
			print("MT: Write message id 0x%02X (%s) with %d data bytes: [%s]" %(mid, getMIDName(mid), length,' '.join("%02X" % ord(v) for v in data)))
	...
	def read_msg(self):
		...
		while (time.time()-start) < self.timeout:
			...
			if 0xFF & sum(data, 0xFF+mid+length+checksum):
				...
				continue
			if self.verbose:
				print( "MT: Got message id 0x%02X (%s) with %d data bytes: [%s]"% (mid, getMIDName(mid), length,' '.join("%02X" % v for v in data)))
			if mid == MID.Error:
	...
	def GetConfiguration(self):
		...
		try:
			masterID, period, skipfactor, _, _, _, date, time, num, deviceID,length, mode, settings = struct.unpack('!IHHHHI8s8s32x32xHIHHI8x', config)
		except struct.error:
	...
	def SetOutputConfiguration(self, output_configuration):
		...
		self._ensure_config_state()
		print("output conf : {}".format(output_configuration))
...
def main():
	...
	try:
		...
		# 	return 1
		# open device
		print("Config device : {}, baudrate : {}".format(device,baudrate))
		try:
		...
		if 'change-baudrate' in actions:
			print("Changing baudrate from %d to %d:" % (baudrate, new_baudrate),sys.stdout.flush())
			mt.ChangeBaudrate(new_baudrate)
			print(" Ok")  # should we test that it was actually ok?
		if 'reset' in actions:
			print("Restoring factory defaults",sys.stdout.flush())
			mt.RestoreFactoryDefaults()
			print(" Ok")  # should we test that it was actually ok?
		if 'configure' in actions:
			print("Changing output configuration",sys.stdout.flush())
		...
		if 'echo' in actions:
			...
			try:
				while True:
					...
					print("Mode : {}, Setting : {}".format(mode,settings))
					print(mt.read_measurement(mode, settings))
					print(type(mt.read_measurement(mode, settings)))
					# data
					# time.sleep(5)
